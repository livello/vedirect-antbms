	...
	while ( (p = strstr(p, "\r\n")) != NULL) {
		...
		if (sscanf(p, "\r\n%ms\t%ms", &fptr->name, &fptr->value) != 2) {
			...
			if (fptr->name != NULL && strcmp(fptr->name, "Checksum") == 0) {
				...
				fprintf(stderr, "Error scanning field in from:%s %02x\n\n", p,p[0]);
				ve_direct_free_block(block);
				return 1;
			}
		};
...
ve_direct_block_t *ve_direct_parse_block(char *block_text_in)
{
	...
	_ve_direct_add_field(block, "ERR", ve_direct_value_type_int, "", "Error code (0 no error)");
	...
	_ve_direct_add_field(block, "CS", ve_direct_value_type_int, "", "Charge Status");	
	_ve_direct_add_field(block, "H19", ve_direct_value_type_int, "DWh", "Total Yield");	
	_ve_direct_add_field(block, "H20", ve_direct_value_type_int, "DWh", "Yield Today");	
	_ve_direct_add_field(block, "H21", ve_direct_value_type_int, "W", "Max Power Today");	
